 ...
 */
Putter.prototype.handle = function (err, res, callback) {
    if (err) return this.emit('error', err);
    if(200 == res.statusCode) {
        callback(res.req);
        this.emit('response', { path: res.req.url });
    } else {
        callback('failed status code')
        this.emit('error', new Error('Could not save attachment file, status code: ' + res.statusCode));
    }
};

Putter.prototype.put = function (string, path, mime, permission, callback) {
    ...
    var ws = this.client.putStream(
      ...
      , headers
      , function (err, res) {
        return this.handle.bind(this, err, res, callback)
      }
    );
